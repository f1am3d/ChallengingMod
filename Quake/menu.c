...
void
M_DrawTransPicTranslate(int x, int y, qpic_t * pic, plcolour_t top, plcolour_t bottom) //johnfitz -- more parameters
{
...
//
//  // Эмуляция какой-либо работы или ожидания
...
static void rgbtohsv(byte * rgb, vec3_t result) {  //helper for the setup menu
...
//http://axonflux.com/handy-rgb-to-hsl-and-rgb-to-hsv-color-model-c
static void hsvtorgb(float inh, float s, float v, byte * out) {  //helper for the setup menu
  int r, g, b;
  ...
  switch (i) {
    default:
    case 0:
      r = v * 0xff, g = t * 0xff, b = p * 0xff;
      break;
    case 1:
      r = q * 0xff, g = v * 0xff, b = p * 0xff;
      break;
    case 2:
      r = p * 0xff, g = v * 0xff, b = t * 0xff;
      break;
    case 3:
      r = p * 0xff, g = q * 0xff, b = v * 0xff;
      break;
    case 4:
      r = t * 0xff, g = p * 0xff, b = v * 0xff;
      break;
    case 5:
      r = v * 0xff, g = p * 0xff, b = q * 0xff;
      break;
  }
...
void M_Keys_Populate(void) {
  ...
  if (!numbindnames) {
    ...
    while (numbindnames < NUMQUAKECOMMANDS) {
      bindnames[numbindnames].cmd = strcpy(Z_Malloc(strlen(quakebindnames[numbindnames][0]) + 1),
                                           quakebindnames[numbindnames][0]);
      bindnames[numbindnames].desc = strcpy(Z_Malloc(strlen(quakebindnames[numbindnames][1]) + 1),
                                            quakebindnames[numbindnames][1]);
      numbindnames++;
    }
  }
    ...
    case K_UPARROW:
      ...
      do {
        ...
        if (keys_cursor >= numbindnames) {
          if (keys_first &&
              strcmp(bindnames[keys_first].cmd, "-")) {  //some weirdness, so the user can re-view any leading titles
...
static void M_Extras_AdjustSliders(int dir) {
  ...
  switch (extras_cursor) {
    case EXTRAS_FILTERING:
      ...
      if (m == 0) {
        Cvar_Set("gl_texturemode",
                 "nll");  //use linear minification filter to reduce distant noise without uglifying the visuals.
    ...
    case EXTRAS_PREDICTION:
      ...
      if ((m & 3) == 2) {
        m += dir;
      } //boo! don't like that combo. skip it
      m &= 3;
...
void M_Draw(void) {
  if (cls.menu_qcvm.extfuncs.m_draw) {  //Spike -- menuqc
    ...
    if (scr_con_current && key_dest ==
                           key_menu) {  //make sure we don't have the console getting drawn in the background making the menu unreadable.
    ...
    glEnable(
      GL_BLEND);  //in the finest tradition of glquake, we litter gl state calls all over the place. yay state trackers.
    glDisable(
      GL_ALPHA_TEST);  //in the finest tradition of glquake, we litter gl state calls all over the place. yay state trackers.
