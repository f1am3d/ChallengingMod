...
cvar_t max_edicts = {"max_edicts", "15000",
                     CVAR_NONE}; //johnfitz //ericw -- changed from 2048 to 8192, removed CVAR_ARCHIVE
...
void Host_Error(const char * error, ...) {
  ...
  }  //equivelent to drawresetcliparea, to reset any damage if we crashed in csqc.
...
static void CL_LoadCSProgs(void) {
  ...
  PR_SwitchQCVM(&cl.qcvm);
  if (pr_checkextension.value && !cl_nocsqc.value) {  //only try to use csqc if qc extensions are enabled.
    ...
    if (
      (*versionedname && PR_LoadProgs(versionedname, false, PROGHEADER_CRC, pr_csqcbuiltins, pr_csqcnumbuiltins) &&
       (qcvm->extfuncs.CSQC_DrawHud || cl.qcvm.extfuncs.CSQC_UpdateView)) ||
      (PR_LoadProgs("csprogs.dat", false, PROGHEADER_CRC, pr_csqcbuiltins, pr_csqcnumbuiltins) &&
       (qcvm->extfuncs.CSQC_DrawHud || qcvm->extfuncs.CSQC_DrawScores || cl.qcvm.extfuncs.CSQC_UpdateView)) ||
      ...
      if ((*versionedname && qcvm->progshash == csqchash && qcvm->progssize == csqcsize) || cls.demoplayback) {
      ...
      else {  //okay, it doesn't match. full csqc is disallowed to prevent cheats, but we still allow simplecsqc...
        if (!qcvm->extfuncs.CSQC_DrawHud) {  //no simplecsqc entry points... abort entirely!
          PR_ClearProgs(qcvm);
          PR_SwitchQCVM(NULL);
          return;
        }
        ...
        qcvm->extfuncs
          .CSQC_Ent_Update = 0;    //don't let the qc know where ents are... the server should prevent this, but make sure the user didn't cheese a 'cmd enablecsqc'
        qcvm->extfuncs.CSQC_Ent_Remove = 0;
        qcvm->extfuncs
          .CSQC_Parse_StuffCmd = 0;  //don't allow blocking stuffcmds... though we can't prevent cvar queries+sets, so this is probably futile...

        qcvm->extglobals
          .clientcommandframe = NULL;  //input frames are blocked, so don't bother to connect these either.
  ...
  else {  //always initialsing at least part of it, allowing us to share some state with prediction.
    qcvm->worldmodel = cl.worldmodel;
    SV_ClearWorld();
  }
  PR_SwitchQCVM(NULL);
...
void Host_Init(void) {
  ...
  if (setjmp (host_abortserver)) {
    return;
  }      // don't do the above twice if the following Cbuf_Execute does bad things.
