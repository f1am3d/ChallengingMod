...
void CL_SendInitialUserinfo(void * ctx, const char * key, const char * val) {
  if (*key == '*') {
    return;
  }  //servers don't like that sort of userinfo key
  if (!strcmp(key, "name"))
...
static qboolean CL_LerpEntity(entity_t * ent, vec3_t org, vec3_t ang, float frac) {
  ...
  if (ent->netstate.pmovetype && ent - cl.entities == cl.viewentity && qcvm->worldmodel &&
      !cl_nopred.value) {  //note: V_CalcRefdef will copy from cl.entities[viewent] to get its origin, so doing it here is the proper place anyway.
    static struct {
      int seq;
      float waterjumptime;
...
//		memset(&pmove, 0xff, sizeof(pmove));
    ...
    if (j < cl.movemessages - countof(cl.movecmds)) {
      j = cl.movemessages - countof(cl.movecmds);
    }  //don't corrupt things, lost is lost.

    if (propagate[j % countof(cl.movecmds)].seq == j) {  //some things can only be known thanks to propagation.
...
// if the object wasn't included in the last packet, remove it
    if (ent->msgtime != cl.mtime[0]) {
      ...
      ent->lerpflags |= LERP_RESETMOVE |
                        LERP_RESETANIM; //johnfitz -- next time this entity slot is reused, the lerp will need to be reset
      continue;
    }
...
// rotate binary objects locally
    ...
    if (cl.paused) {}
      ...
      PScript_ParticleTrail(oldorg, ent->origin, cl.particle_precache[ent->netstate.traileffectnum].index, frametime, i,
                            axis, &ent->trailstate);
    }
...
int CL_GenerateRandomParticlePrecache(const char * pname) {  //for dpp7 compat
  size_t i;
  pname = va("%s", pname);
...
void CL_Download_Data(void) {
  ...
  if (!cls.download.file) {
    return;
  }  //demo started mid-record? something weird anyway
...
qboolean CL_CheckDownload(const char * filename) {
  if (sv.active) {
    return false;
  }  //no point downloading if we're the server...
  if (*filename == '*')
...
qboolean CL_CheckDownloads(void) {
  int i;
  if (cl.model_download == 0 && cl.model_count &&
      cl.model_name[1]) {  //haxors, download the lit first, but only if we don't already have the bsp
  ...
  if (!cl.worldmodel && cl.model_count >= 2) {
    ...
    if (cl.protocol == PROTOCOL_VERSION_DP7) {
      ...
      cl.protocol_particles = true;
    }  //doesn't have a pext flag of its own, but at least we know what it is.
#endif
  }
...
static void CL_UserinfoChanged(scoreboard_t * sb) {
  ...
  sb->spectator = atoi(Info_GetKey(sb->userinfo, "*spectator", tmp,
                                   sizeof(tmp)));  //0=regular player, 1=spectator, 2=spec-with-scores aka waiting their turn to (re)spawn.
...
static void SV_DecodeUserInfo(client_t * client) {
  ...
  if (Q_strcmp(client->name, tmp) != 0) {  //name changed.
...
void SV_UpdateInfo(int edict, const char * keyname, const char * value) {
  ...
  if (strcmp(value, oldvalue)) {  //its changed. actually broadcast it.
    ...
    if (*keyname == '_' || !sv.active) {
      return;
    }  //underscore means private (user) keys. these are not networked to clients.
...
static void CL_LegacyColor_f(
  void) {  //spike -- code to handle the legacy _cl_color cvar (we now use separate qw-style topcolor/bottomcolor userinfo cvars)
...
void CL_Init(void) {
  ...
  Cvar_RegisterAlias(&cl_name,
                     "_cl_name");  //spike -- for compat with configs now that 'name' is a cvar in its own right.
  Cvar_RegisterVariable(&cl_topcolor);
  Cvar_RegisterVariable(&cl_bottomcolor);
  Cmd_AddCommand ("_cl_color",
                  CL_LegacyColor_f);  //for loading vanilla configs (we have separate qw-style topcolor/bottomcolor userinfo cvars instead)
  ...
  Cmd_AddCommand_ServerCommand ("paknames",
                                CL_ServerExtension_Ignore_f); //package names in use by the server (including gamedir+extension)
