...
void CL_Record_Prespawn(void) {
  ...
  for (idx = 0; idx < cl.num_entities; idx++) {
    ...
    if (net_message.cursize > 4096) {  //periodically flush so that large maps don't need larger than vanilla limits
      CL_WriteDemoMessage();
      SZ_Clear(&net_message);
    }
  }
  ...
  for (idx = 1; idx < cl.num_statics; idx++) {
    MSG_WriteStaticOrBaseLine(&net_message, -1, &cl.static_entities[idx]->baseline, cl.protocol_pext2, cl.protocol,
                              cl.protocolflags);
  ...
  for (i = NUM_AMBIENTS; i < total_channels; i++) {
    ...
    if (ss->entnum || ss->entchannel) {
      continue;
    }  //can't have been a static sound
...
void CL_Record_Spawn(void) {
  ...
  for (i = 0; i < MAX_CL_STATS; i++) {
    ...
    if ((double) cl.stats[i] != cl.statsf[i] && (unsigned int) cl.stats[i] <=
                                                0x00ffffff) {  //if the float representation seems to have more precision then use that, unless its getting huge in which case we're probably getting fpu truncation, so go back to more compatible ints
...
void CL_Record_f(void) {
  ...
  if (c == 2 && cls.state == ca_connected) {
    ...
    switch (cl.protocol) {
      ...
      default:
        Con_Printf(
          "Can not record - protocol not supported for recording mid-map\nClient demo recording must be started before connecting\n");
        return;
    }
  }
