...
void
CL_SpawnSpriteEffect(vec3_t org/*, vec3_t dir, vec3_t orientationup*/, qmodel_t * model, int startframe, int framecount,
                     float framerate/*, float alpha, float scale, float randspin, float gravity, int traileffect, unsigned int renderflags, int skinnum*/) {
  if (startframe < 0) {
...
void CL_ParseTEnt(void) {
  ...
  if (cl.qcvm.extfuncs.CSQC_Parse_TempEntity && !cl.qcvm
    .nogameaccess) {  //this is likely to be misused, but I'm implementing it anyway because DP doesn't allow anything better and consistency is generally easier than simplicity. You should generally be using CSQC_Parse_Event instead, for custom messages.
  ...
  switch (type) {
    ...
    case TE_SUPERSPIKE:      // super spike hitting wall
      ...
      if (PScript_RunParticleEffectTypeString(pos, NULL, 1,
                                              (type == TEDP_SUPERSPIKEQUAD) ? "TE_SUPERSPIKEQUAD" : "TE_SUPERSPIKE")) {
    ...
    case TE_GUNSHOT:      // bullet hitting wall
      ...
      pos[1] = MSG_ReadCoord(cl.protocolflags);
      pos[2] = MSG_ReadCoord(cl.protocolflags);
      if (PScript_RunParticleEffectTypeString(pos, NULL, rnd,
                                              (type == TEDP_GUNSHOTQUAD) ? "TE_GUNSHOTQUAD" : "TE_GUNSHOT")) {
    ...
    case TE_EXPLOSION:      // rocket explosion
      ...
      pos[2] = MSG_ReadCoord(cl.protocolflags);
      if (PScript_RunParticleEffectTypeString(pos, NULL, 1,
                                              (type == TEDP_EXPLOSIONQUAD) ? "TE_EXPLOSIONQUAD" : "TE_EXPLOSION")) {
      ...
      dl->die = cl.time + 0.5;
      dl->decay = 300;
      if (type == TENEH_EXPLOSION3) {  //the *2 is to match dp's expectations, for some reason.
      ...
      if (type == TEFTE_EXPLOSION_SPRITE) {
        ...
        CL_SpawnSpriteEffect(pos/*, NULL, NULL*/, mod, 0, 0,
                             10/*, mod->type==mod_sprite?-1:1, 1, 0, 0, P_INVALID, 0, 0*/);
      }
      break;
...
void CL_ParseEffect(qboolean big) {
  ...
  CL_SpawnSpriteEffect(org/*, NULL, NULL*/, mod, startframe, framecount,
...
void CL_UpdateTEnts(void) {
  ...
  } //johnfitz -- freeze beams when paused
...
// update lightning
  for (i = 0, b = cl_beams; i < MAX_BEAMS; i++, b++) {
    ...
    if (!PScript_ParticleTrail(b->start, b->end, PScript_FindParticleType(b->trailname), host_frametime, b->entity,
                               NULL, &b->trailstate))
      continue;
