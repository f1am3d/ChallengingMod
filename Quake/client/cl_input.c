...
void CL_SendMove(const usercmd_t * cmd) {
  ...
  if (cmd) {
    ...
    if (cl.protocol == PROTOCOL_VERSION_DP7) {
      ...
      MSG_WriteShort(&buf, cl.movemessages &
                           0xffff);  //server will ack this once it has been applied to the player's entity state
      MSG_WriteFloat(&buf,
                     cmd->servertime);  // so server can get cmd timing (pings will be calculated by entframe acks).
    }
    else {
    ...
    for (i = 0; i < 3; i++) {
      ...
      if ((cl.protocol == PROTOCOL_NETQUAKE || cl.protocol == PROTOCOL_VERSION_BJP3) &&
          !NET_QSocketGetProQuakeAngleHack(cls.netcon) && !(cl.protocol_pext2 & PEXT2_PREDINFO)) {
    ...
    if (cl.movemessages <= 2) {
      ...
      cl.movecmds[(cl.movemessages - 1) &
                  MOVECMDS_MASK].seconds = 0;  //and don't predict forwards. should fix prediction going weird at the start of the map.
    }
    else {
    ...
    S_Voip_Transmit(clcfte_voicechat, NULL);
  }/*Spike: Add voice data (with cl_voip_test anyway)*/
