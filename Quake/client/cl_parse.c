...
static unsigned int CLFTE_ReadDelta(unsigned int entnum, entity_state_t * news, const entity_state_t * olds,
...
//		Con_Printf("%3i: Reset %i @ %i\n", msg_readcount, entnum, cls.netchan.incoming_sequence);
    *news = *baseline;
    ...
    if (sv.active) {  //for extra debug info
      qcvm_t * old = qcvm;
      qcvm = NULL;
      PR_SwitchQCVM(&sv.qcvm);
      Con_DPrintf("New entity %i(%s / %s) without reset\n", entnum, PR_GetString(EDICT_NUM(entnum)->v.classname),
                  PR_GetString(EDICT_NUM(entnum)->v.model));
      PR_SwitchQCVM(old);
    }
    else {
...
//	news->movement[2] = 0;
  ...
  if (bits & UF_PREDINFO) {
    ...
    if (predbits &
        UFP_MSEC) {  //the msec value is how old the update is (qw clients normally predict without the server running an update every frame)
  ...
  if (bits & UF_SOLID) {  //knowing the size of an entity is important for prediction
...
static void CL_EntitiesDeltaed(void) {
  ...
  for (newnum = 1; newnum < cl.num_entities; newnum++) {
    ...
    if (ent->msgtime == cl.mtime[0])
      ...
      //johnfitz -- lerping
      if (ent->msgtime + 0.2 <
          cl.mtime[0]) //more than 0.2 seconds since the last message (most entities think every 0.1 sec)
...
static void CLFTE_ParseEntitiesUpdate(void) {
  ...
  if (cl.protocol_pext2 & PEXT2_PREDINFO) {
    int seq = (cl.movemessages & 0xffff0000) |
              (unsigned short) MSG_ReadShort();  //an ack from our input sequences. strictly ascending-or-equal
    if (seq > cl.movemessages) {
      seq -= 0x10000;
    }  //check for cl.movemessages overflowing the low 16 bits, and compensate.
  ...
  if (newtime != cl.mtime[0]) {  //don't mess up lerps if the server is splitting entities into multiple packets.
  ...
  for (;;) {
    ...
    else if (ent->update_type) {  //simple update
      CLFTE_ReadDelta(newnum, &ent->netstate, &ent->netstate, &ent->baseline);
    }
    else {  //we had no previous copy of this entity...
  ...
  if (cl.protocol_pext2 & PEXT2_PREDINFO) {  //stats should normally be sent before the entity data.
    ...
    if (v_gunkick.value == 1) {  //truncate away any extra precision, like vanilla/qs would.
    ...
    if (v_punchangles[0][0] != cl.punchangle[0] || v_punchangles[0][1] != cl.punchangle[1] ||
...
void CLFTE_ParseCSQCEntitiesUpdate(void) {
  if (qcvm->extfuncs.CSQC_Ent_Update) {
    ...
    for (;;) {
      ...
      else {  //otherwise just a 16bit value, with the high bit used as a 'remove' flag
        entnum = (unsigned short) MSG_ReadShort();
        removeflag = !!(entnum & 0x8000);
        entnum &= ~0x8000;
      }
...
static void
CLDP_ReadDelta(unsigned int entnum, entity_state_t * s, const entity_state_t * olds, const entity_state_t * baseline) {
...
static void CL_ParseStartSoundPacket(void) {
  ...
  if (cl.qcvm.extfuncs.CSQC_Event_Sound && cl.sound_precache[sound_num] &&
      !cl.qcvm.nogameaccess) {  //blocked with csqc, too easy to do dead-reckoning.
    qboolean ret = false;
...
// parse protocol version number
  for (;;) {
    ...
    if (i == PROTOCOL_FTE_PEXT1) {
      ...
      if (cl.protocol_pext1 & ~PEXT1_ACCEPTED_CLIENT) {
        Host_Error("Server returned FTE1 protocol extensions that are not supported (%#x)",
                   cl.protocol_pext1 & ~PEXT1_SUPPORTED_CLIENT);
      }
      continue;
    }
    if (i == PROTOCOL_FTE_PEXT2) {
      ...
      if (cl.protocol_pext2 & ~PEXT2_ACCEPTED_CLIENT) {
        Host_Error("Server returned FTE2 protocol extensions that are not supported (%#x)",
  ...
  if (cl.protocol == PROTOCOL_RMQ) {
    const unsigned int supportedflags = (PRFL_SHORTANGLE | PRFL_FLOATANGLE | PRFL_24BITCOORD | PRFL_FLOATCOORD |
...
#define netstate_start offsetof(entity_state_t, scale)
  memcpy((char *) &ent->netstate + offsetof(entity_state_t, modelindex),
         (const char *) &ent->baseline + offsetof(entity_state_t, modelindex),
...
static void CL_ParseBaseline(entity_t * ent, int version) //johnfitz -- added argument
{
  ...
  if (bits &
      B_SCALE) {  //not actually valid in 666, but reading anyway for servers that don't distinguish properly. The warning will have to suffice.
...
static void CL_ParseClientdata(void) {
  ...
  if (cl.protocol == PROTOCOL_VERSION_DP7) {  //dpp7 doesn't really send much here, instead using deltas.
...
void CL_RegisterParticles(void) {
  ...
  }  //make sure this is implicitly loaded.
...
static void CL_ParseStuffText(const char * msg) {
  ...
  for (; (str = strchr(cl.stuffcmdbuf, '\n')); memmove(cl.stuffcmdbuf, str, Q_strlen(str) + 1)) {
    ...
    if (*cl.stuffcmdbuf == 0x01 && cl.protocol ==
                                   PROTOCOL_NETQUAKE) //proquake message, just strip this and try again (doesn't necessarily have a trailing \n straight away)
    {
      for (str = cl.stuffcmdbuf + 1; *str >= 0x01 && *str <= 0x1f; str++) {}//FIXME: parse properly
      continue;
    }
...
static qboolean CL_ParseSpecialPrints(const char * printtext) {
  ...
  if (cl.printtype == PRINT_PINGS) {
    ...
    if (t != n && *n == ' ' && e[-1] == '\n') {
      ...
      if (!strcmp(n, "unconnected\n")) {
        return true;
      }  //just ignore unconnecteds, too many dupes etc
  ...
  if (!cls.demoplayback && *printtext == 1 && e - printtext > 13 &&
      (!strcmp(e - 12, ": f_version\n") || !strcmp(e - 12, ": q_version\n"))) {
...
static void CL_ParseCenterPrint(const char * msg) {
  char * tmp;
  if (cl.qcvm.extfuncs.CSQC_Parse_CenterPrint) {  //let the csqc do it.
    PR_SwitchQCVM(&cl.qcvm);
    tmp = PR_GetTempString();
...
//
  ...
  while (1) {
    ...
    switch (cmd) {
      default:
        Host_Error("Illegible server message %s, previous was %s", svc_strings[cmd],
      ...
      case 52:
        if (cl.protocol == PROTOCOL_VERSION_DP7) {  //svcdp_effect
          CL_ParseEffect(false);
        }
        else {  //2021 release: svc_achievement
          str = MSG_ReadString();
